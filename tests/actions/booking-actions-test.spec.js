...
describe('booking-actions', () => {
  ...
  describe('displayBookingResults', () => {
    it('has the correct type', () => {
      const action = actions.displayBookingResults()

      expect(action.type).to.equal(types.DISPLAY_BOOKING_RESULTS)
    })
  })
  ...
  describe('setAvailabilityView', () => {
    it('has the correct type', () => {
      const action = actions.setAvailabilityView()

      expect(action.type).to.equal(types.SET_AVAILABILITY_VIEW)
    })

    it('has the correct payload', () => {
      const action = actions.setAvailabilityView('compact')

      expect(action.payload).to.equal('compact')
    })
  })
